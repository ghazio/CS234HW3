        ...
        while num_episodes or t < self.config.batch_size:
            ...
            for step in range(self.config.max_ep_len):
                ...
                action = self.policy.act(states[-1][None])[0]
                    #print(f"The step {step} by state {states[-1][None]} works for {episode} perfectly and generates the action {action} with summed value of {np.sum(states[-1][None],axis=1)}")
                #except:
                #print(f"The error is being caused in {step} by state {states[-1][None]} of {episode}with summed value of {np.sum(states[-1][None],axis=1)}")
                pdb.set_trace()
